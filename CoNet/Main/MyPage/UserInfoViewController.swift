...
/*
 ...
 class UserInfoViewController: UIViewController {
     ...
     let cameraButton = UIButton().then { $0.backgroundColor = .clear }
     
     let cameraImage = UIImageView().then {
         $0.image = UIImage(named: "cameraButton")
     }
     ...
     let myPageList = MyPageList()
     lazy var nameView = myPageList.arrowView(title: "이안진")
     ...
     override func viewDidLoad() {
         ...
         nameView.addTarget(self, action: #selector(showChangeNameViewController(_:)), for: .touchUpInside)
     }
